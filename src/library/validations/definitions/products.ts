...
export const inventoryAddFormInputSchema = z.object({
	...
	description: z
		.string()
		.max(serviceConstraints.maximumProductDescriptionCharacters, {
			message: `Description must be less than ${serviceConstraints.maximumProductDescriptionCharacters} characters`,
		})
		.pipe(validCharacters())
		.optional()
		.nullable(),

	priceInMinorUnits: z.string().refine((val) => val === '' || !Number.isNaN(Number(val)), {

...
import { BaseOrder, BrowserOrderItem, DangerousBaseUser, OrderItem, OrderMade, OrderReceived, OrdersFunctionReturnType, Product } from "@/types"
import { convertEmptyToUndefined } from "./arrays"


function mapItemsToOrder({
    order,
    orderItems,
    products,
    businessName,
    ...
    if (returnType === 'ordersReceived') return { orderReceived }
...
export function mapOrders({orders, orderItems, merchants, customers, products, returnType}: MapOrdersProps): {ordersMade?: OrderMade[], ordersReceived?: OrderReceived[]} {

...
export async function checkUserExists(
	...
	const [existingUser]: DangerousBaseUser[] = await database
		.select()
		.from(users)
...
export async function checkMerchantProfileExists(
	userId: number
): Promise<{ merchantProfileExists: boolean; slug: string }> {
...
export async function checkActiveSubscriptionOrTrial(
	...
	const [validFreeTrial] = await database
		...
		.from(freeTrials)
		.where(
			and(gt(freeTrials.endDate, new Date()), eq(freeTrials.userId, userId))
		)
		.limit(1);

export const websiteCopy = {
  heroSection: {
    h1: `Order management made simple`,
    intro: `Simple Order is an order management website for wholesalers and their customers so you can reduce costs, save time, and eliminate phone call confusion. Start a free 30-day trial now and get started in minutes.`,
  },
  metadata: {
    titles: {
      home41: 'Simple Order | Wholesale order management',
    },
    descriptions: {
      home138:
        'A B2B order management website for wholesalers and their customers so you can reduce costs, save time, and eliminate phone call confusion.',
    },
  },
  extras: {
    siteNameWithTag: 'Simple Order | Wholesale Order Management',
    selfContainedDescription: `Simple Order is an order management website for wholesalers and their customers so you can reduce costs, save time, and eliminate phone call confusion.`,
  },
  linkDescriptions: {
    howItWorks: `Learn more about our B2B order management website`,
  },
  CTAs: {
    primary: {
      displayText: `Start free trial`,
      href: '/free-trial',
    },
    secondary: {
      displayText: `How it works`,
      href: '/articles/how-it-works',
    },
  },
} as const

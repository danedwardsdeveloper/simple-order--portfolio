...
export async function POST(request: NextRequest): Promise<NextResponse<InventoryAddPOSTresponse>> {
	...
	try {
		...
		await database.transaction(async (tx) => {
			const existingProducts = await tx.select().from(products).where(eq(products.merchantProfileId, extractedUserId))
			...
			const [addedProduct]: Product[] = await tx.insert(products).values(newProduct).returning()
